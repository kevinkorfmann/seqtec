...
\begin{sourcebox}
The structural foundation begins with the original double-helix report \cite{watson1953}. Later mechanistic and numerical statements require their own primary sources; this citation does not by itself validate the entire chapter.
\end{sourcebox}
...
\textbf{Tautomeric forms} of nucleobases---rare structural isomers in which a proton shifts between nitrogen and oxygen atoms---are of particular significance. When a base transiently adopts its rare tautomer, it can form non-canonical base pairs (e.g., enol-T pairs with G instead of A, imino-C pairs with A instead of G). This is one plausible molecular mechanism of spontaneous mutation during DNA replication. A single universal ``tautomeric mispairing rate'' should not be quoted: the abundance and lifetime of rare states, polymerase selectivity, proofreading, and mismatch repair all affect the observed error rate.
