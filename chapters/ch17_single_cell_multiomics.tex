...
\small
\begin{tabularx}{\textwidth}{>{\bfseries\raggedright}p{2.3cm} X c c c}
\toprule
\textbf{Method} & \textbf{Approach} & \textbf{Uses ATAC} & \textbf{Scalability} & \textbf{Output} \\
\midrule
SCENIC+ & GRNBoost2 regression + cisTarget motif filtering & Yes & High ($>$100K cells) & TF regulons with enhancer links \\
\addlinespace
CellOracle & Bayesian regression with TF binding site priors from ATAC & Yes & Moderate & Directed GRN with perturbation simulation \\
\addlinespace
Dictys & Context-specific GRN using joint RNA + ATAC with dynamic modelling & Yes & Moderate & Dynamic, context-specific GRN \\
\addlinespace
GENIE3 & Random forest regression of TF--gene relationships & No & Low--Moderate & Undirected TF--gene importance scores \\
\addlinespace
GRaNIE & Integrates peak--gene correlations with TF motif enrichment & Yes & High & TF--peak--gene triplets \\
\addlinespace
FigR & DORC-based regulatory inference linking TF activity to gene modules & Yes & High & TF--DORC--gene regulatory circuits \\
\bottomrule
\end{tabularx}
\end{table}
