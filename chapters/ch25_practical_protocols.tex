...
\begin{keyconcept}
This chapter provides detailed, step-by-step laboratory protocols for the complete nanopore sequencing workflow---from sample collection and DNA extraction through library preparation, flow cell loading, and sequencing. Each protocol has been optimised for reliability and is presented in a format that can be printed and brought directly to the bench. Where previous chapters discussed the \emph{theory} behind library preparation (\cref{ch:libprep}) and experimental design (\cref{ch:expdesign}), this chapter provides the \emph{practice}: precise volumes, incubation times, decision trees, and troubleshooting tables to ensure success on the first attempt.
\end{keyconcept}
...
The Centres for Disease Control and Prevention (CDC) and the World Health Organisation (WHO) classify laboratories into four \textbf{Biosafety Levels} (BSL) based on the risk posed by the organisms handled:
...
Align metadata with GSC MIxS (Minimum Information about any (x) Sequence) checklists and \acrshort{SRA} submission requirements from the outset---recording these at collection saves hours of retroactive data entry. See \cref{ch:datastd} for details on data standards.
\end{tipbox}
