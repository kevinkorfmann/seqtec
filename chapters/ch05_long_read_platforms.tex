...
Pacific Biosciences (Menlo Park, California) commercialised single-molecule real-time (\gls{SMRTseq}) sequencing in 2011 with the RS system. The technology has undergone continuous improvement through the RS~II, Sequel, Sequel~II, Sequel~IIe, Revio, and most recently the Vega system. PacBio was acquired by and subsequently merged operations with the former Illumina competitor Omniome, whose sequencing-by-binding (SBB) chemistry now forms the basis of PacBio's short-read instrument, the Onso.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Throughput:} approximately 90~Gb of HiFi reads per SMRT Cell; with four SMRT Cell positions running in parallel, up to 360~Gb per day.
  \item \textbf{Movie time:} 24 hours per SMRT Cell (standard).
  \item \textbf{Cost:} approximately \$800--1{,}000 per SMRT Cell; approximately \$1{,}000--1{,}500 for a 30$\times$ human \gls{WGS} (HiFi).
...
The Vega, announced in 2024 and entering the market in 2025, is PacBio's next-generation benchtop system.

\begin{itemize}[nosep]
  \item \textbf{Positioning:} designed for laboratories seeking HiFi data at lower capital cost and smaller scale.
  \item \textbf{Throughput:} lower than Revio per run but sufficient for individual genome projects, targeted long-read panels, and isoform sequencing.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Output per flow cell:} up to 290~Gb.
  \item \textbf{Total output:} up to approximately 14~Tb (P48, all flow cells).
  \item \textbf{Run time:} typically 72 hours per flow cell.
  \item \textbf{Cost:} approximately \$600--900 per flow cell; approximately \$1{,}000--2{,}000 for a 30$\times$ human \gls{WGS}.
...
Throughput/cell & ${\sim}$90\gigabase\ HiFi & ${\sim}$290\gigabase\ (PromethION) \\
Throughput/instrument & 360\gigabase/day (4 cells) & up to 14\,Tb (P48) \\
Run time & 24\,h per cell & 72\,h per flow cell \\
Cost per Gb & \$10--15 (HiFi) & \$3--8 \\
Cost per 30$\times$ genome & \$1{,}000--1{,}500 & \$1{,}000--2{,}000 \\
Instrument cost & ${\sim}$\$600K (Revio) & ${\sim}$\$250K (P24); ${\sim}$\$450K (P48) \\
